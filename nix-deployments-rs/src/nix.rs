...
pub fn nix_build(config_name: &str, repo_path: &str, commit_hash: &str) -> Result<String> {
    let nix_build = Command::new("nix")
        .current_dir(repo_path)
        .arg("build")
        .arg(format!(".#nixosConfigurations.{}.config.system.build.qcow2", config_name))
        .arg("--out-link")
        .arg(format!("{}/{}/{}/result", repo_path, commit_hash, config_name))
        .output()?;
    if !nix_build.status.success() {
        return Err(AppError::CmdError(format!("Nix build has failed with the exit code: {:?}", nix_build.status.code())))
    }
